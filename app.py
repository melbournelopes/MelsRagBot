...
class FileAssistant:
    ...
    def process_uploaded_files(self, uploaded_files):
        ...
        for uploaded_file in uploaded_files:
            ...
            if total_text_length < self.CHUNK_SIZE:
                st.error(
                    f"The document '{uploaded_file.name}' is too small ({total_text_length} characters). It must be at least {self.CHUNK_SIZE} characters.")
    ...
    def get_retrieval_chain(self, docs):
        ...
        contextualize_prompt = ChatPromptTemplate.from_messages(
            [("system", self.CONTEXTUALIZE_PROMPT), ("human", "{input}")]
        )
        history_aware_retriever = create_history_aware_retriever(self.llm, retriever, contextualize_prompt)

        qa_prompt = ChatPromptTemplate.from_messages(
            [("system", self.QA_SYSTEM_PROMPT), MessagesPlaceholder("chat_history"), ("human", "{input}")]
        )
...
# Create an instance of FileAssistant
...
if uploaded_files and "conversational_rag_chain" not in st.session_state:
    with st.spinner("Processing files..."):  # Show spinner
        document_chunks = assistant.process_uploaded_files(uploaded_files)
        retrieval_chain = assistant.get_retrieval_chain(document_chunks)
        conversational_rag_chain = assistant.create_conversational_chain(retrieval_chain)
...
if user_input:
    if "conversational_rag_chain" not in st.session_state:
        ...
        session_history = assistant.get_session_history(st.session_state["session_id"])
